...
def main(args, prompts):
    ...
    adapter = TextAdapter.from_pretrained(os.path.join(args.ckpt_dir, "adapter")).to(torch_device)
    tokenizer.pad_token = '[PAD]'

    # LoRA
    monkeypatch_or_replace_lora_extended(
        vis, 
        torch.load(os.path.join(args.ckpt_dir, "lora_vis.pt")), 
        r=32, 
        target_replace_module=VIS_REPLACE_MODULES,
    )
    monkeypatch_or_replace_lora_extended(
        text_encoder, 
        torch.load(os.path.join(args.ckpt_dir, "lora_text.pt")), 
        r=32, 
